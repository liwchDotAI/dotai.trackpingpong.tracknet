...
csv_path = parameters['label_path']
ouput_path = parameters['output_path']

opt = keras.optimizers.Adadelta(learning_rate=1.0)
model=ResNet_Track(input_shape=(3, HEIGHT, WIDTH))

# TODO
model.compile(loss=[binary_focal_loss(alpha=.25, gamma=2)], optimizer=opt, metrics=[keras.metrics.BinaryAccuracy()])
...
out = cv2.VideoWriter(ouput_path+video_name+'_predict.mp4', fourcc, fps, size)
out.write(image)
